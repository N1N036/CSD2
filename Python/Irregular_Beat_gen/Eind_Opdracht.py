...
trig = 0
#Kick
Bk = rand.randrange(4, 20)#Lengte van de loop
Nk = rand.randrange(2, Bk)#aantal hits

#Snare
Bs = rand.randrange(4, 20)#Lengte van de loop
Ns = rand.randrange(2, Bs)#aantal hits

#Clap
Bc = rand.randrange(4, 20)#Lengte van de loop
Nc = rand.randrange(2, Bc)#aantal hits
...
def write():
    ...
    t.sleep(numPlaybackTimes)#wait till the program is done
...
#generating the on/off values in their respective arrays
...
def play():
    ...
    while numPlaybackTimes >= 0:
        ...
        i = i + 1
        multi = 60 / (bpm) #multiplying value
        trig = 0
        intervalk = euclidianGen(Bk, Nk)
        intervals = euclidianGen(Bs, Ns)
        intervalc = euclidianGen(Bc, Nc)
