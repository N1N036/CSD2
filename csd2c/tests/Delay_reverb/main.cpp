...
fVolumes[0] = 1.0;
  ...
  jack.onProcess = [&ringbuffer1, &saw, &sine1, &sine2, &white, &samplerate, &CurrentTimeS, &CurrentSamp, &adsr1, &adsr2](jack_default_audio_sample_t *inBuf,
    ...
    for(unsigned int i=0;i<nframes;i++)
    {
      ...
      if (synthState == 1)
      {
        ...
        ringbuffer1.push
        (
          saw.getSample() * 0.1 * adsr1.getValue(CurrentTimeS)
        );
        saw.tick();
        sine1.tick();
      ...
      else
      {
        ...
        sine1.phaseMod
        (
          saw.getSample()*0.02*adsr1.getValue(CurrentTimeS) + saw.getSample()*0.01*sine2.getSample()
        );
        sine1.tick();
        sine2.tick();
        saw.tick();
      ...
      //std::cout << (iPreBuf- 1000) % 100000 << std::endl;



      //outBuf[i] = preBuf[(iPreBuf + 100000 - 1) % 100000]; //creating the delay effect

      //outBuf[i] = ringbuffer1.getSample(0) + ringbuffer1.getSample(900);
      //outBuf[i] = ringbuffer1.getSample(0) + ringbuffer1.getSample(300) + ringbuffer1.getSample(3000);
      outBuf[i] = ringbuffer1.getSamples(iDelays,fVolumes);
      //outBuf[i] = outBuf[i] *10;
      //std::cout << ringbuffer1.getsample(0);
    }
